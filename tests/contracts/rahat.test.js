...
describe('Rahat', () => {
	...
	beforeAll(async () => {
		...
		testContract = await test.instance.deploy({ data: test.bytecode }).send({ from: accounts[0], gas: 1500000 });
		...
		rahatAdminContract = await rahatAdmin.instance
			.deploy({
				data: rahatAdmin.bytecode,
				arguments: [tokenContract._address, rahatContract._address, 1000000, accounts[0]]
			})
		...
		await rahatAdminContract.methods
			.setProjectBudget(projectId, Number(projectCapital))
	...
	it('should issue token to given number', async () => {
		...
		await rahatContract.methods
			.issueToken(projectId, 9841602388, issueAmount)
			.send({ from: accounts[0], gas: 1000000 });
	...
	it('should claim token from registered beneficiary', async () => {
		...
		await rahatContract.methods
			.createClaim(beneficiary, Number(claimAmount))
			.send({ from: vendorAccount, gas: 1000000 });
	...
	it('should approve claim requested from vendors', async () => {
		...
		await rahatContract.methods
			.approveClaim(vendorAccount, beneficiary, otpHash, timeToLive)
			.send({ from: accounts[0], gas: 1000000 });
	...
	it("should not get tokens if OTP didn't match", async () => {
		const getTokenTx = async () => {
			try {
				await rahatContract.methods
					.getTokensFromClaim(beneficiary, '1010')
					.send({ from: vendorAccount, gas: 1000000 });
			} catch (e) {
