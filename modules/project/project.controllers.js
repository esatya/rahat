...
const Project = {
  ...
  async changeStatus(id, payload) {
    ...
    const project = await ProjectModel.findOneAndUpdate(
      { _id: id, is_archived: false }, { status, updated_by }, { new: true, runValidators: true },
    );
    // TODO implement blockchain function using project._id
    return project;
  },
  ...
  async addTokenAllocation(id, payload) {
    ...
    await ProjectModel.findOneAndUpdate({ _id: id }, {
  ...
  async confirmTokenAllocation(allocationId, txhash) {
    return ProjectModel.findOneAndUpdate({ 'allocations._id': Types.ObjectId(allocationId) },
      {
        $set: {
  ...
  async remove(id, currentUser) {
    ...
    const project = await ProjectModel.findOneAndUpdate(
      { _id: id }, { is_archived: true }, { new: true, runValidators: true },
    );
...
module.exports = {
  ...
  addTokenAllocation: (req) => Project.addTokenAllocation(
    req.params.id, req.payload,
  ),
